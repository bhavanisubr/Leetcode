 ...
 * ask your friend to guess it. Each time your friend guesses a number, you give a hint. The hint tells your 
 * friend how many digits are in the correct positions (called "bulls") and how many digits are in the wrong 
 * positions (called "cows"). Your friend will use those hints to find out the secret number.
 *
 * For example:
 * Secret number:  "1807"
 * Friend's guess: "7810"
...
public:
  string getHint(string secret, string guess) {
    ...
    for (int i = 0; i < secret.size(); i++)
      if (secret[i] != '-')  ++charCountMap[secret[i]];
    for (int i = 0; i < guess.size(); i++) {
      if (guess[i] == '-')  continue;
      auto it = charCountMap.find(guess[i]);
      if (it != charCountMap.end() && it->second) {
        ++cows;
        --(it->second);
      }
    }
    ...
    string out;
    out += to_string(bulls) + "A" + to_string(cows) + "B";
    return out;
  }
};
